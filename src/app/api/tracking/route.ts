...
export async function GET(request: Request) {
    try {
        ...
        if (!shipRes.ok) {
            ...
            return NextResponse.json({ success: false, error: `Supabase query error: ${errText}` }, { status: 500 });
        }
        ...
        // 7. Check outbound_lmd_bags table
        ...
        // isStep2Done evaluated strictly based on service_provider_allocation.mapped_zone !== null
        ...
        const isStep4Done = Boolean(allocation?.unsealed || bagUnsealRecord);
        ...
        const step5Dt = formatDate(outboundBagRecord?.created_at || allocation?.updated_at);
        ...
        const steps = [
            ...
            },
            {
                ...
                date: isStep2Done ? step2Dt.date : '-',
                time: isStep2Done ? step2Dt.time : '-',
                location: isStep2Done ? (shipment.consignee_location_name || shipment.consignee_address_3 || 'Destination Zone') : 'Pending Allocation',
                activity: 'Zone Allocation',
                notes: isStep2Done ? `Assigned Zone: ${zoneName}` : 'Pending Zone Allocation (mapped_zone is null)',
        ...
        else if (isStep2Done) overallStatus = 'Zone Allocated';
        ...
        const shipmentInfo = {
            ...
            weight: shipment.weight ? `${shipment.weight} ${shipment.weight_measure || 'kg'}` : '-',
            deadWeight: shipment.dead_weight ? `${shipment.dead_weight} kg` : '-',
            cubicWeight: shipment.customer_declared_weight ? `${shipment.customer_declared_weight} kg` : '-',
