...
export default function ManifestTrackingTab({
    ...
    const getLocalDateString = (dateInput?: string | Date | null): string => {
        if (!dateInput) return '';
        try {
    ...
    // Filter manifests and unassigned bags by selectedDateFilter
    const dateFilteredData = useMemo(() => {
        const allManifests = manifestTrackingData?.manifests || [];
        const allUnassignedBags = manifestTrackingData?.unassignedBags || [];

        if (selectedDateFilter === 'ALL') {
            return {
                manifests: allManifests,
                unassignedBags: allUnassignedBags,
                stats: manifestTrackingData?.stats || {
                    totalManifests: allManifests.length,
                    openManifests: allManifests.filter((m: any) => m.status === 'OPEN').length,
                    closedManifests: allManifests.filter((m: any) => m.status === 'CLOSED').length,
                    totalBags: allManifests.reduce((acc: number, m: any) => acc + (m.bags?.length || 0), 0) + allUnassignedBags.length,
                    openBags: 0,
                    sealedBags: 0,
                    manifestedBags: 0,
                    totalParcels: 0,
        ...
        const filteredManifests = allManifests.filter((m: any) => {
            const mDate = getLocalDateString(m.created_at || m.opened_at || m.closed_at);
            if (mDate === selectedDateFilter) return true;
            const hasBagOnDate = (m.bags || []).some((b: any) => getLocalDateString(b.created_at || b.opened_at || b.sealed_at) === selectedDateFilter);
            return hasBagOnDate;
        });

        const filteredUnassignedBags = allUnassignedBags.filter((b: any) => {
            const bDate = getLocalDateString(b.created_at || b.opened_at || b.sealed_at);
            return bDate === selectedDateFilter;
        });

        // Compute dynamic stats for the selected date
        const allDateBags: any[] = [];
        filteredManifests.forEach((m: any) => {
            (m.bags || []).forEach((b: any) => allDateBags.push(b));
        });
        filteredUnassignedBags.forEach((b: any) => allDateBags.push(b));

        const totalManifests = filteredManifests.length;
        const openManifests = filteredManifests.filter((m: any) => m.status === 'OPEN').length;
        const closedManifests = filteredManifests.filter((m: any) => m.status === 'CLOSED').length;

        const totalBags = allDateBags.length;
        const openBags = allDateBags.filter((b: any) => b.status === 'OPEN').length;
        const sealedBags = allDateBags.filter((b: any) => b.status === 'SEALED').length;
        const manifestedBags = allDateBags.filter((b: any) => b.new_manifest_reference || b.is_bag_in_a_manifest).length;

        const totalParcels = allDateBags.reduce((acc: number, b: any) => acc + (b.parcel_count || b.parcels?.length || 0), 0);
        const totalWeight = allDateBags.reduce((acc: number, b: any) => acc + (Number(b.total_weight) || 0), 0);

        const partnerStats: Record<string, { bags: number; parcels: number; weight: number }> = {};
        allDateBags.forEach((b: any) => {
            ...
            partnerStats[partner].parcels += b.parcel_count || b.parcels?.length || 0;
            partnerStats[partner].weight += Number(b.total_weight) || 0;
        });

        return {
            manifests: filteredManifests,
            unassignedBags: filteredUnassignedBags,
            stats: {
                totalManifests,
                openManifests,
                closedManifests,
    ...
    }, [manifestTrackingData, selectedDateFilter]);

    const formatDisplayDate = (dateStr: string) => {
        if (dateStr === 'ALL') return 'All Dates (Full History)';
        if (dateStr === todayStr) return `Today (${new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })})`;
        try {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {isLoadingManifestTracking && !manifestTrackingData ? (
                ...
                <>
                    {/* 0. Top Date Filter & Quick Switcher Banner */}
                    <div style={{
                        ...
                        {/* Left: Active Date Status Chip */}
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            <div style={{
                                width: '32px',
                                height: '32px',
                                borderRadius: '8px',
                                backgroundColor: selectedDateFilter === todayStr ? '#fef2f2' : '#f3f4f6',
                                color: selectedDateFilter === todayStr ? '#b91c1c' : '#374151',
                                display: 'flex',
                                alignItems: 'center',
                            ...
                            <div>
                                ...
                                <div style={{ fontSize: '15px', fontWeight: '800', color: '#111827', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    <span>{formatDisplayDate(selectedDateFilter)}</span>
                                </div>
                            </div>
                        </div>

                        {/* Right: Datepicker Input */}
                    ...
                    {/* 1. Top KPI Summary Stats Grid (Date Filtered) */}
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px' }}>
                        ...
                        <div style={{ backgroundColor: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '20px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                            ...
                            <div style={{ fontSize: '28px', fontWeight: '800', color: '#111827', marginBottom: '8px', lineHeight: '1' }}>
                                {dateFilteredData?.stats?.totalManifests || 0}
                            </div>
                            <div style={{ display: 'flex', gap: '8px', fontSize: '12px' }}>
                                <span style={{ backgroundColor: '#ffffff', border: '1px solid #b91c1c', color: '#b91c1c', padding: '2px 8px', borderRadius: '12px', fontWeight: '600' }}>
                                    {dateFilteredData?.stats?.openManifests || 0} OPEN
                                </span>
                                <span style={{ backgroundColor: '#ffffff', border: '1px solid #b91c1c', color: '#b91c1c', padding: '2px 8px', borderRadius: '12px', fontWeight: '600' }}>
                                    {dateFilteredData?.stats?.closedManifests || 0} CLOSED
                                </span>
                            </div>
                        </div>
                        ...
                        <div style={{ backgroundColor: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '20px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                            ...
                            <div style={{ fontSize: '28px', fontWeight: '800', color: '#111827', marginBottom: '8px', lineHeight: '1' }}>
                                {dateFilteredData?.stats?.totalBags || 0}
                            </div>
                            <div style={{ display: 'flex', gap: '6px', fontSize: '11px', flexWrap: 'wrap' }}>
                                <span style={{ backgroundColor: '#ffffff', border: '1px solid #b91c1c', color: '#b91c1c', padding: '2px 8px', borderRadius: '12px', fontWeight: '600' }}>
                                    {dateFilteredData?.stats?.openBags || 0} Open
                                </span>
                                <span style={{ backgroundColor: '#ffffff', border: '1px solid #b91c1c', color: '#b91c1c', padding: '2px 8px', borderRadius: '12px', fontWeight: '600' }}>
                                    {dateFilteredData?.stats?.sealedBags || 0} Sealed
                                </span>
                                <span style={{ backgroundColor: '#ffffff', border: '1px solid #b91c1c', color: '#b91c1c', padding: '2px 8px', borderRadius: '12px', fontWeight: '600' }}>
                                    {dateFilteredData?.stats?.manifestedBags || 0} In Manifest
                                </span>
                            </div>
                        </div>
                        ...
                        <div style={{ backgroundColor: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '20px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                            ...
                            <div style={{ fontSize: '28px', fontWeight: '800', color: '#111827', marginBottom: '8px', lineHeight: '1' }}>
                                {dateFilteredData?.stats?.totalParcels || 0}
                            </div>
                            <div style={{ fontSize: '12px', color: '#6b7280', fontWeight: '500' }}>
                                Total Weight: <strong style={{ color: '#111827' }}>{formatKg(dateFilteredData?.stats?.totalWeight)} kg</strong>
                            </div>
                        </div>
                        ...
                        <div style={{ backgroundColor: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '16px 20px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                {Object.entries(dateFilteredData?.stats?.partnerStats || {}).map(([partner, pStats]: [string, any]) => (
                                ...
                                {Object.keys(dateFilteredData?.stats?.partnerStats || {}).length === 0 && (
                                    <div style={{ fontSize: '12px', color: '#9ca3af', fontStyle: 'italic' }}>No partner data for this date</div>
                                )}
                            </div>
                        </div>
                    </div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            ...
                            <button
                                onClick={() => {
                                    ...
                                    const filtered = (dateFilteredData?.manifests || []).filter((m: any) => {
                                ...
                                disabled={isLoadingManifestTracking || !dateFilteredData?.manifests?.length}
                                style={{
                                    display: 'inline-flex',
                                    alignItems: 'center',
                                    gap: '6px',
                                    ...
                                    fontSize: '12px',
                                    fontWeight: '600',
                                    cursor: (!dateFilteredData?.manifests?.length || isLoadingManifestTracking) ? 'not-allowed' : 'pointer',
                                    opacity: (!dateFilteredData?.manifests?.length || isLoadingManifestTracking) ? 0.6 : 1
                                }}
                            >
                    ...
                    <div style={{ backgroundColor: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '20px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                            <div style={{ fontSize: '15px', fontWeight: '700', color: '#111827' }}>
                                Outbound Manifests Directory ({dateFilteredData?.manifests?.length || 0})
                            </div>
                        ...
                        {(() => {
                            const q = manifestTrackingSearchQuery.trim().toLowerCase();
                            const filteredManifests = (dateFilteredData?.manifests || []).filter(m => {
                                if (manifestTrackingStatusFilter !== 'ALL' && m.status !== manifestTrackingStatusFilter) return false;
                                if (manifestTrackingPartnerFilter !== 'ALL' && m.service_provider_name?.toLowerCase() !== manifestTrackingPartnerFilter.toLowerCase()) {
                                    const hasMatchingBag = m.bags?.some((b: any) => b.target_partner?.toLowerCase() === manifestTrackingPartnerFilter.toLowerCase());
                                    if (!hasMatchingBag) return false;
                                ...
                                return (
                                    <div style={{ textAlign: 'center', padding: '40px 20px', color: '#6b7280', fontSize: '13px' }}>
                                        <div style={{ fontSize: '14px', fontWeight: '600', color: '#374151' }}>
                                            No outbound manifests found for {formatDisplayDate(selectedDateFilter)}.
                                        </div>
                                    </div>
                                );
                            }
                    ...
                    {dateFilteredData?.unassignedBags && dateFilteredData.unassignedBags.length > 0 && (
                        <div style={{ backgroundColor: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '12px', padding: '20px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                                <div style={{ fontSize: '15px', fontWeight: '700', color: '#111827' }}>
                                    Standalone / Unassigned Outbound Bags ({dateFilteredData.unassignedBags.length})
                                </div>
                                <div style={{ fontSize: '12px', color: '#6b7280' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                {dateFilteredData.unassignedBags.map((bag: any) => {
