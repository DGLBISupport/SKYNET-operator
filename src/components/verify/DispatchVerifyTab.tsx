...
export default function DispatchVerifyTab({
    ...
    const formatScanTimeHelper = (isoString?: string | null) => {
        if (!isoString) return '-';
        try {
    ...
    const handleExportExcel = () => {
        try {
            if (!verifyScannedParcels || verifyScannedParcels.length === 0) {
                toast.error(`No scanned parcels available to export for ${verifySelectedDate}`);
                return;
            }

            const wb = XLSX.utils.book_new();

            // 1. Scanned Parcels Sheet
            const parcelRows = verifyScannedParcels.map((p: any, idx: number) => ({
                'No.': idx + 1,
                'Parcel Tracking No.': p.trackingNumber || '',
                'Temu / Sender Reference': p.senderReference || p.temuBarcode || '',
                'Inbound Manifest (MAWB Ref)': p.inboundMawb || 'UNASSIGNED',
                'Outbound Bag': p.outboundBag || 'Pending Bag',
                'Outbound Manifest': p.outboundManifest || 'Pending Manifest',
                '1st Scan Time': p.firstScanTime ? formatScanTimeHelper(p.firstScanTime) : (p.unsealed ? 'Unsealed' : 'N/A'),
                '1st Scanned By': p.firstScannedBy || (p.unsealed ? 'Staff' : 'N/A'),
                '2nd Scan Time': p.secondScanTime ? formatScanTimeHelper(p.secondScanTime) : (p.verified ? 'Verified' : 'Pending'),
                '2nd Scanned By': p.secondScannedBy || (p.verified ? 'Staff' : 'Pending'),
                'Allocated Courier': p.serviceProvider || 'Unassigned',
                '1st Scan (Unsealed)': p.unsealed ? 'YES' : 'NO',
                '2nd Scan (Verified)': p.verified ? 'YES' : 'NO',
                'Scan Status': p.scanStatus || (p.verified ? '2ND_SCAN_DONE' : p.unsealed ? '1ST_SCAN_DONE' : 'PENDING'),
                'Raw 1st Scan Timestamp': p.firstScanTime || '',
                'Raw 2nd Scan Timestamp': p.secondScanTime || ''
            }));

            const parcelsWs = XLSX.utils.json_to_sheet(parcelRows);
            parcelsWs['!cols'] = [
                { wch: 6 },
                { wch: 22 },
                { wch: 24 },
            ...
            XLSX.utils.book_append_sheet(wb, parcelsWs, 'Scanned Parcels Log');

            // 2. Daily Summary Sheet
            const summaryRows = [
                { 'Category': 'Report Overview', 'Metric': 'Selected Date', 'Value': verifySelectedDate },
                { 'Category': 'Report Overview', 'Metric': 'Report Generated At', 'Value': new Date().toLocaleString() },
                { 'Category': 'Report Overview', 'Metric': 'Total Scanned Parcels', 'Value': verifyDailyStats?.totalScannedAll || 0 },
                { 'Category': 'Scan Operations', 'Metric': 'Unsealed (1st Scan Done)', 'Value': verifyDailyStats?.unsealed1stScanDone || 0 },
                { 'Category': 'Scan Operations', 'Metric': 'Verified (2nd Scan Done)', 'Value': verifyDailyStats?.verified2ndScanDone || 0 },
                { 'Category': 'Courier Allocation', 'Metric': 'PickMe Allocated', 'Value': verifyDailyStats?.pickMeScanned || 0 },
                { 'Category': 'Courier Allocation', 'Metric': 'Domex Allocated', 'Value': verifyDailyStats?.domexScanned || 0 },
                { 'Category': 'Courier Allocation', 'Metric': 'SITREK Allocated', 'Value': (verifyDailyStats as any)?.sitrekScanned || 0 },
                { 'Category': 'Courier Allocation', 'Metric': 'Pronto Allocated', 'Value': (verifyDailyStats as any)?.prontoScanned || 0 },
                { 'Category': 'Courier Allocation', 'Metric': 'Other / Unassigned', 'Value': (verifyDailyStats as any)?.otherScanned || 0 }
            ];
            ...
            XLSX.utils.book_append_sheet(wb, summaryWs, 'Daily Overview Summary');

            // 3. MAWB Breakdown Sheet (if available)
            if (Array.isArray(verifyDailyStats?.manifestTable) && verifyDailyStats.manifestTable.length > 0) {
                const mawbRows = verifyDailyStats.manifestTable.map((m: any, idx: number) => ({
                    'No.': idx + 1,
                    'Inbound Manifest (MAWB)': m.inboundMawb || 'UNASSIGNED',
                    'Outbound Manifest(s)': m.outboundManifest || 'Pending',
                    'Daily Scanned': m.dailyScanned || 0,
                    '1st Scan (Unsealed)': m.unsealedCount || 0,
                    '2nd Scan (Verified)': m.verifiedCount || 0,
                    'PickMe Count': m.pickMeScanned || 0,
                    'Domex Count': m.domexScanned || 0,
                    'SITREK Count': m.sitrekScanned || 0,
                    'Pronto Count': m.prontoScanned || 0
                }));

                const mawbWs = XLSX.utils.json_to_sheet(mawbRows);
                mawbWs['!cols'] = [
                    { wch: 6 },
                    { wch: 26 },
                    { wch: 40 },
                ...
                XLSX.utils.book_append_sheet(wb, mawbWs, 'MAWB Breakdown');
            }

            const cleanDate = (verifySelectedDate || '').replace(/[^0-9-]/g, '_');
            const filename = `Dispatch_Verification_${cleanDate}.xlsx`;
            XLSX.writeFile(wb, filename);
            toast.success(`Exported ${verifyScannedParcels.length} parcel scan records to ${filename}`);
        } catch (err: any) {
            console.error('Error exporting dispatch verification to Excel:', err);
            toast.error('Failed to export Dispatch Verification report to Excel');
        }
    };
            ...
            <div style={{ ...card, marginBottom: '20px', fontFamily: 'var(--font-sans, "Inter", "Inter Fallback", sans-serif)' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', flexWrap: 'wrap', gap: '12px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                        ...
                        {/* Export Excel Button in Table Header */}
                        <button
                            onClick={handleExportExcel}
                            title="Export scanned parcels log to Excel (.xlsx)"
                            style={{
                                backgroundColor: '#15803d',
                                color: '#ffffff',
                                border: '1px solid #166534',
