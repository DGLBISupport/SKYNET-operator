            ...
            try {
                ...
                for (const bag of allBagsList) {
                    ...
                    for (const parcel of parcels) {
                        ...
                        if (!firstShipmentDetail && detail) firstShipmentDetail = detail;
                        ...
                        emit({ type: 'parcel', bagNumber: bag.bagNumber, trackingNumber: trackingNum, index: parcelIdx, total: parcels.length, status: 'ok', hasDetail: !!detail });
                    }
                ...
                const finalManifestPayload = { bag_numbers: allBagNumbers, total_bags: allBagNumbers.length, total_parcels: shipmentBlocks.length, status: 'CLOSED' };
                if (manifestId) {
                ...
                // ── Step 7: Build header info & upload to FFDX ───────────────
                ...
                emit({ type: 'ffdx_start', parcelCount: shipmentBlocks.length });

                let ffdxResult: { success: boolean; response?: string; error?: string } = { success: false, error: 'No parcels to upload' };

                if (shipmentBlocks.length > 0) {
                    const xmlPayload = buildUploadXml(mawbRef, shipmentBlocks, headerInfo);
                    ffdxResult = await postToFfdx(xmlPayload, mawbRef);
                } else {
                    // No parcels — still mark as uploaded
                    ffdxResult = { success: true };
                }

                // Mark uploaded in DB if success
                ...
                emit({
                    ...
                    summary: {
                        totalBags,
                        totalParcels: shipmentBlocks.length,
                        uploaded: ffdxResult.success ? shipmentBlocks.length : 0,
                        errors: ffdxResult.success ? 0 : shipmentBlocks.length,
